...
# Define Network
class Net(nn.Module):
    def __init__(self, sparsity=0.4):
        super().__init__()
        ...
        # 定义掩码矩阵
        self.mask_fc1 = torch.rand_like(self.fc1.weight) > sparsity 
        self.mask_fc2 = torch.rand_like(self.fc2.weight) > sparsity
        self.mask_fc3 = torch.rand_like(self.fc3.weight) > sparsity
    ...
    def forward(self, x):
        ...
        for step in range(num_steps):
            cur1 = F.linear(x[:, step, :], self.fc1.weight * self.mask_fc1, self.fc1.bias)  # Process each time step
            spk1, mem1 = self.lif1(cur1, mem1)
            cur2 = F.linear(spk1, self.fc2.weight * self.mask_fc2, self.fc2.bias)
            spk2, mem2 = self.lif2(cur2, mem2)
            cur3 = F.linear(spk2, self.fc3.weight * self.mask_fc3, self.fc3.bias)
...
net.mask_fc1 = net.mask_fc1.to(device)
net.mask_fc2 = net.mask_fc2.to(device)
net.mask_fc3 = net.mask_fc3.to(device)
