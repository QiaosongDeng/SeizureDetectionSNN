...
% expandedEEGSignals = expandCellArray(eegSignals, 256);
% writematrix(expandedEEGSignals, 'dataset.csv');
...
function expandedArray = expandCellArray(cellArray, numElementsInCell)
    % 获取 cellArray 的行列数
    [numRows, numCols] = size(cellArray); 
    
    % 初始化一个空的数组来存储展开后的数据
    % 展开后的数据维度应该是 (numRows*numElementsInCell) x numCols
    expandedArray = zeros(numRows * numElementsInCell, numCols);
    
    % 遍历 cellArray 中的每个 cell
    for col = 1:numCols
        for row = 1:numRows
            % 获取当前 cell 的数据
            cellData = cellArray{row, col};
            
            % 确保 cellData 是 numElementsInCellx1 的 double 数据
            if isnumeric(cellData) && numel(cellData) == numElementsInCell
                % 计算在展开数组中的起始索引
                startIndex = (row - 1) * numElementsInCell + 1;
                endIndex = startIndex + numElementsInCell - 1;
                
                % 将 cell 数据放入展开后的数组对应位置
                expandedArray(startIndex:endIndex, col) = cellData;
            else
                error('Cell 数据格式不匹配');
            end
        end
