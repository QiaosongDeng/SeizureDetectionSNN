...
class EEGNormalize(object):
    def __call__(self, sample):
        ...
        for i in range(sample.shape[1]):  # sample.shape[1] is 23
            ...
            # Min-max normalization to ensure values are in [0, 1]
            if max_val != min_val:
                normalized_sample[:, i] = (channel_data - min_val) / (max_val - min_val)
            else:
                normalized_sample[:, i] = 0  # If max equals min, all values are the same and can be set to 0
        return normalized_sample

class LatencyTransform(object):
    def __init__(self, num_steps):
        self.num_steps = num_steps

    def __call__(self, sample):
        # sample shape: (256, 23)
        # Apply the latency coding to the sample
        latency_coded_sample = spikegen.latency(sample, num_steps=self.num_steps)
        print(f"After latency_coded_transform: {latency_coded_sample.shape}")
        return latency_coded_sample
    
# Define transformations
data_transforms = transforms.Compose([
    EEGNormalize(),
    transforms.ToTensor(),
    LatencyTransform(num_steps=256), 
...
class EDFDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        print(f"After transform: {sample.squeeze().shape}")  # After transform
