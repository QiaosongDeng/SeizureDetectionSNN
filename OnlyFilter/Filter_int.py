...
class EEGNormalize(object):
    def __call__(self, sample):
        # sample shape: (256, 23)
        normalized_sample = np.zeros_like(sample, dtype=np.float32)
        # Define the scale factor to map the normalized data to [-100000000, 100000000]
        scale_factor = 10**6
        ...
        for i in range(sample.shape[1]):  # sample.shape[1] is 23
            ...
            # print(f'Original data (Channel {i+1}): {channel_data}')  # 打印原始数据

            # Normalize the data to have zero mean and unit variance
            if channel_std != 0:
                normalized_sample[:, i] = (channel_data - channel_mean) / channel_std
            else:
                normalized_sample[:, i] = 0  # Avoid division by zero
            
            # print(f'Normalized data (Channel {i+1}): {normalized_sample[:, i]}')  # 打印规范化后的数据

            # Scale to [-scale_factor, scale_factor] and round to the nearest integer
            normalized_sample[:, i] = np.round(normalized_sample[:, i] * scale_factor).astype(np.int32)
            
            # Make sure all values are within the range [-scale_factor, scale_factor]
            # normalized_sample[:, i] = np.clip(normalized_sample[:, i], -scale_factor, scale_factor)

            # print(f'Scaled data (Channel {i+1}): {normalized_sample[:, i]}')  # 打印缩放后的数据
